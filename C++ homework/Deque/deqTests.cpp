...
void testWithSize() {
  ...
  assert((simple.size() == size_t(size)) &&
         std::all_of(simple.begin(), simple.end(),
...
void testStaticAsserts() {
  ...
  // constructor from int"); static_assert(std::is_constructible_v<Deque<T2>,
  // int>, "should have constructor from int");
...
void testPushAndPop() {
  ...
  assert(std::all_of(d.begin(), d.end(),
  ...
  //  assert(std::count(d.begin(), d.end(), NotDefaultConstructible{2}) ==
  //  11000);
}
...
void testInsertAndErase() {
  ...
  //  == start_size - 1); assert(std::count(d.begin(), d.end(),
  //  NotDefaultConstructible{2}) == 1);
...
///*
    void testStrongGuarantee() {
        ...
        auto is_intact = [&] {
            return d.size() == size && std::all_of(d.begin(), d.end(),
   [initial_data](const auto& item) {return item.data == initial_data;} );
        ...
        try {
            ...
            for (auto it = d.begin(); it != d.end(); ++it) {
              //std::cerr << it->durability << ' ';
            }
            //std::cerr << '\n';
            d.insert(d.begin() + size / 2, Fragile(3, initial_data + 2));
        } catch (...) {
            // might throw depending on the implementation
            // if it DID throw, then deque should be untouchedn
            for (auto it = d.begin(); it != d.end(); ++it) {
              //std::cerr << it->durability << ' ';
            }
            //std::cerr << initial_data <<  ' ' <<  d.begin()->data << '\n';
            assert(is_intact());
        }
    }
//*/
